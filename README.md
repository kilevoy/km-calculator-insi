...
- всего 117 автоматических тестов: 113 frontend/расчётных и 4 security-теста прокси.
...
Workflow `.github/workflows/deploy-pages.yml` собирает и публикует SPA вручную.
Автоматический запуск намеренно отключён, чтобы приватный репозиторий не получал
ошибку deployment на тарифе без поддержки private GitHub Pages.

1. Сделайте репозиторий публичным либо включите тариф с private GitHub Pages.
2. Откройте `Settings → Pages` и выберите источник `GitHub Actions`.
3. Запустите workflow `Deploy GitHub Pages` вручную.
...
Калькулятор, PDF, черновики, URL, тесты, CI и workflow статической публикации готовы. Для полного ввода в корпоративную эксплуатацию остаются внешние операции: разрешить GitHub Pages для репозитория, выдать OAuth/service credentials Google Workspace, назначить целевую папку, развернуть прокси за HTTPS и выполнить приёмочный тест загрузки в реальный Drive.
