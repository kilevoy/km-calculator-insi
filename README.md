...
- всего 128 автоматических тестов: 124 frontend/расчётных и 4 security-теста прокси.
...
- Административный конструктор методики АС: `?admin=as`.
- Редактор рабочей модели коэффициентов КМ: `?admin=km`.

### Редактор модели КМ

Страница `?admin=km` загружает полный эталон коэффициентов Excel R2.0.1 и создаёт
из него независимую рабочую редакцию. Доступны:

- 554 числовых параметра по системам, геометрии и дополнительным элементам;
- поиск, фильтр изменённых значений и возврат отдельного коэффициента к эталону;
- контрольный расчёт с одновременным сравнением Excel и рабочей модели;
- журнал отличий с абсолютным и процентным изменением;
- версия, статус, владелец и причина корректировки;
- локальное сохранение, импорт и экспорт JSON.

Рабочая редакция намеренно не применяется к основному калькулятору автоматически.
Это защищает подтверждённую Excel-совместимость до формального утверждения новой версии.

### Конструктор модели АС

Страница `?admin=as` предназначена для руководителя проектной группы. Она позволяет:

- описать дополнительные исходные поля будущего калькулятора АС;
- настроить состав выдаваемой документации и трудоёмкость;
- связать работы с числовыми параметрами и коэффициентами сложности;
- задать нормо-час, накладные расходы, резерв и минимальную стоимость;
- собрать фактические данные завершённых объектов для калибровки;
- сохранить черновик локально, импортировать и экспортировать модель в JSON.

Конструктор пока не изменяет утверждённый Excel-совместимый расчёт. Новая модель
подключается к рабочему калькулятору после заполнения и утверждения.
